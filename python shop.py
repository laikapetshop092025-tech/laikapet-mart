...
# --- 4. DASHBOARD (FIXED ERRORS & RESTORED BOXES) ---
if menu == "📊 Dashboard":
    ...
    k_df = load_data("CustomerKhata"); i_df = load_data("Inventory")
    
    # Financial Logic (Safe against IndexError)
    ...
    <div style="display: flex; gap: 10px; justify-content: space-around;">
        <div style="background-color: #FFEBEE; padding: 15px; border-radius: 10px; border-left: 8px solid #D32F2F; width: 19%;">
            <p style="color: #D32F2F; margin: 0;">💵 Galla Balance</p> <h3 style="margin: 0;">₹{bc + sc - ec - pc:,.2f}</h3>
        </div>
        <div style="background-color: #E3F2FD; padding: 15px; border-radius: 10px; border-left: 8px solid #1976D2; width: 19%;">
            <p style="color: #1976D2; margin: 0;">🏦 Online Balance</p> <h3 style="margin: 0;">₹{bo + so - eo - po:,.2f}</h3>
        </div>
        <div style="background-color: #F3E5F5; padding: 15px; border-radius: 10px; border-left: 8px solid #7B1FA2; width: 19%;">
            <p style="color: #7B1FA2; margin: 0;">⚡ Mix Total</p> <h3 style="margin: 0;">₹{(bc + sc - ec - pc) + (bo + so - eo - po):,.2f}</h3>
        </div>
    ...
    tp_d = (pd.to_numeric(i_t.iloc[:, 1].astype(str).str.split().str[0], errors='coerce').fillna(0) * pd.to_numeric(i_t.iloc[:, 3], errors='coerce').fillna(0)).sum()
...
# --- 5. BILLING (RESTORED REFERRAL & REDEEM) ---
elif menu == "🧾 Billing":
...
# --- 6. PURCHASE ---
    ...
    with st.expander("📥 Add New Items", expanded=True):
        n = st.text_input("Item Name"); q = st.number_input("Qty", 1.0); u = st.selectbox("Unit", ["Kg", "Pcs", "Pkt"]); r = st.number_input("Rate"); m = st.selectbox("Paid From", ["Cash", "Online", "Pocket"])
        if st.button("➕ Save"):
            save_data("Inventory", [n, f"{q} {u}", "Stock", r, str(today_dt), m]); st.rerun()

# --- 7. LIVE STOCK (TOTAL VALUE RESTORED) ---
...
# --- 11. SUPPLIER DUES ---
    ...
    with st.form("due"):
        s = st.text_input("Supplier"); a = st.number_input("Amt"); t = st.selectbox("Action", ["Maal (+)", "Payment (-)"]); m = st.selectbox("Mode", ["Cash", "Online", "Pocket"])
