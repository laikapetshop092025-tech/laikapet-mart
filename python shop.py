...
# --- 4. DASHBOARD (DAILY DATE & MONTHLY NAME ADDED) ---
if menu == "📊 Dashboard":
    ...
    # Financial Row (Original Line-to-Line)
    ...
    def show_metrics(df_s, df_e, df_i, title, p_type="today"):
        ...
        st.subheader(f"📈 {title} Report"); c1, c2, c3 = st.columns(3)
        c1.metric("Total Sale", f"₹{ts}"); c2.metric("Total Expense", f"₹{te}"); c3.metric("Total Profit", f"₹{tp}")
...
# --- 5. BILLING (DAILY LIST & DELETE) ---
    ...
    # (Original Form Logic Restored Here...)
    with st.form("bill"):
...
# --- 7. PURANA MASTER (EXPENSE, PET, KHATA, LOYALTY, ADMIN) ---
elif menu == "💰 Expenses":
    st.header("💰 Expenses")
