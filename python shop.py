...
def load_data(sheet_name):
    try:
        ...
        # Safe Date Detection to fix KeyError
...
# --- 4. DASHBOARD (FIXED ERRORS & PURCHASE DEDUCTION) ---
if menu == "📊 Dashboard":
...
# --- 5. BILLING (ROYALTY + REFERRAL) ---
    ...
    st.info(f"👑 Royalty Points Available: {pts_bal}")
...
# --- 6. PURCHASE (MODES INCLUDED) ---
elif menu == "📦 Purchase":
    st.header("📦 Purchase Entry")
    with st.expander("📥 Add New Items", expanded=True):
        n = st.text_input("Item Name"); q = st.number_input("Qty", 1.0); u = st.selectbox("Unit", ["Kg", "Pcs", "Pkt"]); r = st.number_input("Rate"); m = st.selectbox("Paid From", ["Cash", "Online", "Pocket", "Hand"])
...
# --- 9. PET REGISTER (FULL DETAILS) ---
    ...
    with st.form("pet"):
        c1, c2 = st.columns(2)
        on = c1.text_input("Owner Name"); ph = c2.text_input("Phone Number"); br = c1.text_input("Breed")
...
# --- 11. SUPPLIER DUES ---
    ...
    with st.form("due"):
        s = st.text_input("Supplier"); a = st.number_input("Amount"); t = st.selectbox("Action", ["Maal (+)", "Payment (-)"]); m = st.selectbox("Mode", ["Cash", "Online", "Hand"])
