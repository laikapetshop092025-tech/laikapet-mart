...
if menu == "📊 Dashboard":
    ...
    # Purchase Calculation (Safe Method to avoid Index Error)
    pc = 0; po = 0
    ...
    <div style="display: flex; gap: 10px; justify-content: space-around;">
        ...
        <div style="background-color: #E3F2FD; padding: 15px; border-radius: 10px; border-left: 8px solid #1976D2; width: 48%;">
    ...
    c1, c2 = st.columns(2)
    c1.metric("Today's Sale", f"₹{ts_d}")
    c2.metric("Monthly Sale", f"₹{pd.to_numeric(s_df[s_df['Date'].dt.month == curr_m].iloc[:, 3], errors='coerce').sum() if not s_df.empty else 0}")

# --- 5. BILLING (REDEEM & DELETE ADDED) ---
elif menu == "🧾 Billing":
    st.header("🧾 Billing")
    inv_df = load_data("Inventory"); s_df = load_data("Sales")
    c_name = st.text_input("Customer Name"); c_ph = st.text_input("Phone Number"); pay_m = st.selectbox("Payment Mode", ["Cash", "Online", "Udhaar"])
...
# --- 7. LIVE STOCK (VALUE ADDED) ---
    ...
    if not i_df.empty:
        ...
        stock_sum['Value'] = stock_sum.iloc[:, 1] * stock_sum.iloc[:, 2]
        total_stock_val = stock_sum['Value'].sum()
        st.subheader(f"💰 Total Stock Value: ₹{total_stock_val:,.2f}")
        for _, row in stock_sum.iterrows():
            if row.iloc[1] < 2: st.error(f"🚨 {row.iloc[0]}: {row.iloc[1]} Left (Value: ₹{row['Value']})")
            else: st.info(f"✅ {row.iloc[0]}: {row.iloc[1]} Left (Value: ₹{row['Value']})")

# --- 8. PET REGISTER (OWNER PHONE ADDED) ---
elif menu == "🐾 Pet Register":
    st.header("🐾 Pet Register")
    breeds = ["Labrador", "GSD", "Shih Tzu", "Pug", "Persian Cat", "Indie", "Other"]
    with st.form("pet"):
        c1, c2 = st.columns(2)
        on = c1.text_input("Owner Name"); oph = c2.text_input("Owner Phone")
...
# --- 9. CUSTOMER KHATA ---
    ...
    with st.form("kh"):
        name = st.text_input("Name"); amt = st.number_input("Amount"); t = st.selectbox("Type", ["Udhaar", "Jama"])
        if st.form_submit_button("Save Entry"):
            save_data("CustomerKhata", [name, amt if "Udhaar" in t else -amt, str(today_dt), "N/A"]); st.rerun()
    st.divider(); st.subheader("📋 Pending List")
    k_df = load_data("CustomerKhata")
    if not k_df.empty: st.table(k_df.groupby(k_df.columns[0]).agg({k_df.columns[1]: 'sum'}).reset_index())

# --- 10. ADMIN SETTINGS (SUPPLIER ONLY) ---
elif menu == "⚙️ Admin Settings":
    st.header("⚙️ Supplier Dues")
    with st.form("due"):
        comp = st.text_input("Supplier Name"); amt = st.number_input("Amount"); m = st.selectbox("Mode", ["Cash", "Online", "Pocket"])
        if st.form_submit_button("Save Payment"):
            save_data("Dues", [comp, -amt, str(today_dt), m]); st.rerun()
    d_df = load_data("Dues")
    if not d_df.empty:
        st.divider(); st.subheader("📑 Pending Supplier List")
