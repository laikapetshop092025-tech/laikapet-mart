...
if menu == "📊 Dashboard":
    ...
    <div style="display: flex; gap: 10px; justify-content: space-around;">
        <div style="background-color: #FFEBEE; padding: 15px; border-radius: 10px; border-left: 8px solid #D32F2F; width: 19%;">
            <p style="color: #D32F2F; margin: 0;">💵 Galla (Cash)</p> <h3 style="margin: 0;">₹{bc + sc - ec - pc:,.2f}</h3>
        </div>
        <div style="background-color: #E3F2FD; padding: 15px; border-radius: 10px; border-left: 8px solid #1976D2; width: 19%;">
            <p style="color: #1976D2; margin: 0;">🏦 Online (Bank)</p> <h3 style="margin: 0;">₹{bo + so - eo - po:,.2f}</h3>
        </div>
    ...
    # MONTHLY SUMMARY (CHAARON CHEEZEIN)
    st.divider(); st.subheader(f"🗓️ Monthly Summary")
    s_m = s_df[s_df['Date'].apply(lambda x: x.month if hasattr(x, 'month') else 0) == curr_m] if not s_df.empty else pd.DataFrame()
    i_m = i_df[i_df['Date'].apply(lambda x: x.month if hasattr(x, 'month') else 0) == curr_m] if not i_df.empty else pd.DataFrame()
    e_m = e_df[e_df['Date'].apply(lambda x: x.month if hasattr(x, 'month') else 0) == curr_m] if not e_df.empty else pd.DataFrame()
...
# --- 5. BAAKI TABS (NO LOGIC CHANGE) ---
    ...
    with st.expander("🛒 Add Item"):
        ...
        if st.button("➕ Add"):
            ...
            st.session_state.bill_cart.append({"Item": it, "Qty": f"{q} {u}", "Price": p, "Profit": (p-pur_r)*q})
            st.rerun()
    if st.session_state.bill_cart:
        ...
        if st.button("✅ Save"):
            for item in st.session_state.bill_cart:
                save_data("Sales", [str(today_dt), item['Item'], item['Qty'], item['Price'], "Cash", c_ph, 0, item['Profit']])
    ...
    with st.expander("📥 Add Item"):
        n = st.text_input("Item Name"); q = st.number_input("Qty", 1.0); u = st.selectbox("Unit", ["Pcs", "Kg", "Pkt"]); r = st.number_input("Rate")
        if st.button("➕ Add Item"):
            st.session_state.pur_cart.append({"Item": n, "Qty": f"{q} {u}", "Rate": r}); st.rerun()
...
# --- 📋 LIVE STOCK (ALERT LOGIC ADDED) ---
    ...
    with st.form("due"):
        comp = st.text_input("Supplier"); amt = st.number_input("Amount"); t = st.selectbox("Type", ["Maal (+)", "Payment (-)"]); m = st.selectbox("Mode", ["Cash", "Online", "Pocket"])
