...
if menu == "📊 Dashboard":
    ...
    <div style="display: flex; gap: 10px; justify-content: space-around;">
        ...
        <div style="background-color: #E3F2FD; padding: 15px; border-radius: 10px; border-left: 8px solid #1976D2; width: 24%;">
            <p style="color: #1976D2; margin: 0;">🏦 Online (Bank)</p> <h2 style="margin: 0;">₹{base_online + sale_online + khata_online - exp_online:,.2f}</h2>
        </div>
...
# --- 5. BILLING ---
    ...
    with st.form("bill"):
        ...
        if st.form_submit_button("SAVE BILL"):
            pts_add = int(((q*pr)/100) * (5 if is_weekend else 2))
            if redeem: pts_add = -pts_bal
            save_data("Sales", [str(today_dt), it, f"{q} {unit}", q*pr, mode, f"{c_name} ({ph})", pts_add, (pr-pur_rate)*q])
...
# --- 6. PURCHASE (DAILY LIST RESTORED) ---
elif menu == "📦 Purchase":
    ...
    if not i_df.empty:
        ...
        for i, row in i_df[i_df['Date'].dt.date == today_dt].iterrows():
            ...
            if c2.button("❌", key=f"del_i_{i}"): delete_row("Inventory", i); st.rerun()

# --- 7. CUSTOMER KHATA (LIST RESTORED) ---
...
# --- OTHER SECTIONS (PET, LIVE STOCK, EXPENSES, LOYALTY) ---
elif menu == "📋 Live Stock":
    st.header("📋 Live Stock Alerts")
    i_df = load_data("Inventory"); s_df = load_data("Sales")
    # Stock calculation logic...
    st.divider(); st.download_button("📥 Download Stock List", i_df.to_csv(index=False), "stock.csv")
    ...
    st.header("💰 Expenses")
    # Miscellaneous cat restored exactly...
