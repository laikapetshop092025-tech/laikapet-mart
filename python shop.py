...
# --- 6. PURCHASE (DAILY LIST ADDED) ---
    ...
    if not i_df.empty:
        st.subheader(f"📑 Today's Purchase List ({today_dt})")
        # Sirf aaj ki purchase dikhayega
...
# --- 7. LIVE STOCK (DOWNLOAD BUTTON RESTORED) ---
    ...
    if not i_df.empty:
        ...
        st.download_button("📥 Download Full Stock List", stock.to_csv(index=False), "stock_report.csv")

# --- 8. PET REGISTER, EXPENSES, KHATA, LOYALTY, ADMIN (Line-to-Line Same) ---
