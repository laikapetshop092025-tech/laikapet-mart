...
# --- 7. PET REGISTER ---
    ...
    with st.form("pet"):
        c1, c2 = st.columns(2)
        with c1: cn = st.text_input("Customer Name"); ph = st.text_input("Phone Number"); br = st.selectbox("Breed", dog_breeds + ["Other"])
...
# --- 8. ADMIN SETTINGS (PAYMENT OPTIONS & DELETE ADDED) ---
    ...
    st.divider()
    st.subheader("🏢 Company Hisab-Kitab")
    with st.form("due"):
        comp = st.text_input("Company Name")
        type = st.selectbox("Transaction Type", ["Stock Udhaar Liya (+)", "Company ko Payment Diya (-)"])
