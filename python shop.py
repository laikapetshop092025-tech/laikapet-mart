...
# --- 4. DASHBOARD (REPORTS, GRAPH & STOCK COST ADDED) ---
if menu == "📊 Dashboard":
    ...
    if not i_df.empty:
        ...
        total_stock_cost = (temp_i.iloc[:, 1] * temp_i.iloc[:, 3]).sum()
    ...
    <div style="display: flex; gap: 10px; justify-content: space-around;">
        ...
        <div style="background-color: #E8F5E9; padding: 15px; border-radius: 10px; border-left: 8px solid #388E3C; width: 24%;">
            <p style="color: #388E3C; margin: 0;">📦 Stock Value</p> <h2 style="margin: 0;">₹{total_stock_cost:,.2f}</h2>
        </div>
    </div>
    ...
    c1.metric("Sale", f"₹{ts_d}"); c2.metric("Expense", f"₹{te_d}"); c3.metric("Profit", f"₹{tprof_d}")

    # Sales Graph
    st.divider(); st.subheader("📊 Sales Trend (Last 7 Days)")
    if not s_df.empty:
...
# --- 5. BAAKI SECTIONS (NO CHANGES) ---
    ...
    if st.session_state.bill_cart:
        for i, item in enumerate(st.session_state.bill_cart):
            ...
            c1.write(f"{item['Item']} - {item['Qty']} x {item['Price']}")
    ...
    if st.session_state.pur_cart:
        for i, item in enumerate(st.session_state.pur_cart):
            c1, c2 = st.columns([4, 1])
            c1.write(f"{item['Item']} - {item['Qty']} x {item['Rate']}")
