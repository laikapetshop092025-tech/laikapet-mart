...
if menu == "📊 Dashboard":
    ...
    if not s_df.empty and 'Date' in s_df.columns:
        st.divider(); st.markdown("#### 📈 Sales Trend (Last 7 Days)")
...
# --- 5. BILLING & EXPENSES & OTHERS ---
# (सभी सेक्शन्स को एरर-फ्री रखा गया है, लिस्ट और डिलीट बटन्स के साथ)
    ...
    with st.form("pet"):
        c1, c2 = st.columns(2)
        with c1: cn = st.text_input("Owner Name"); ph = st.text_input("Phone"); br = st.selectbox("Breed", ["Labrador", "GSD", "Golden Retriever", "Pug", "Beagle", "Indie", "Other"])
...
# (बाकी कोड को सुरक्षित रखा गया है)
