...
# --- 4. DASHBOARD (CASH, ONLINE, UDHAAR, CHART) ---
if menu == "📊 Dashboard":
    ...
    <div style="display: flex; gap: 10px; justify-content: space-around;">
        <div style="background-color: #FFEBEE; padding: 15px; border-radius: 10px; border-left: 8px solid #D32F2F; width: 24%;">
            <p style="color: #D32F2F; margin: 0;">💵 Cash (Galla)</p> <h2 style="margin: 0;">₹{base_cash + sale_cash + khata_cash - exp_cash:,.2f}</h2>
        </div>
        ...
        <div style="background-color: #E8F5E9; padding: 15px; border-radius: 10px; border-left: 8px solid #388E3C; width: 24%;">
            <p style="color: #388E3C; margin: 0;">💰 Net Balance</p> <h2 style="margin: 0;">₹{(base_cash + sale_cash + khata_cash - exp_cash + base_online + sale_online + khata_online - exp_online):,.2f}</h2>
        </div>
    </div>
    ...
    show_metrics(s_df, i_df, e_df, "Today", "today")
    st.divider()
    if not s_df.empty:
        fig = px.line(s_df.groupby(s_df['Date'].dt.date).agg({s_df.columns[3]: 'sum'}).reset_index().tail(7), x='Date', y=s_df.columns[3], title="7 Days Sales Chart")
        st.plotly_chart(fig, use_container_width=True)

# --- 5. BILLING (REDEEM, REFERRAL, DAILY LIST & DELETE) ---
elif menu == "🧾 Billing":
    ...
    with st.form("bill"):
        ...
        with c3: 
            ...
            redeem = st.checkbox(f"Redeem {pts_bal} Pts?"); is_ref = st.checkbox("Referral Bonus? (+10 Pts)")
...
# --- 6. PET REGISTER (MONTHS DROP-DOWN & VAX DATES) ---
elif menu == "🐾 Pet Register":
    st.header("🐾 Pet Register")
    breeds = ["Labrador", "GSD", "Pug", "Shih Tzu", "Persian Cat", "Indie Dog", "Other"]
    with st.form("pet"):
        c1, c2 = st.columns(2)
        with c1: on = st.text_input("Owner Name"); ph = st.text_input("Phone Number"); br = st.selectbox("Breed", breeds)
        with c2: 
            age = st.selectbox("Age In", [f"{i} Months" for i in range(1,12)] + [f"{i} Years" for i in range(1,15)])
...
# --- 7. LIVE STOCK (RED ALERT & DOWNLOAD) ---
    ...
    if not i_df.empty:
        ...
        for _, r in stock.iterrows():
            if r['Rem'] <= 2: st.error(f"❌ ALERT: {r['Item']} Only {r['Rem']} {r['Unit']} Left")
...
# --- 8. EXPENSES (MISCELLANEOUS CATEGORY RESTORED) ---
elif menu == "💰 Expenses":
    st.header("💰 Expenses")
...
# --- 9. CUSTOMER KHATA (CASH/ONLINE JAMA LOGIC) ---
elif menu == "📒 Customer Khata":
    st.header("📒 Udhaar & Payment")
    with st.form("kh"):
        name = st.text_input("Customer Name/Phone"); amt = st.number_input("Amount")
        t = st.selectbox("Type", ["Baki (Udhaar)", "Jama (Payment)"])
        p_mode = st.selectbox("Payment Mode (If Jama)", ["Cash", "Online", "N/A"])
        if st.form_submit_button("Save Entry"):
            final_amt = -amt if "Jama" in t else amt
            save_data("CustomerKhata", [name, final_amt, str(today_dt), p_mode]); st.rerun()
    k_df = load_data("CustomerKhata")
    if not k_df.empty:
        st.subheader("Udhaar Summary")
...
# --- 10. LOYALTY CLUB (NAME + PHONE) ---
    ...
    if not s_df.empty:
        ...
        loyalty.columns = ['Customer (Name & Phone)', 'Points Balance']
        st.dataframe(loyalty[loyalty.iloc[:, 1] > 0], use_container_width=True)

# --- 11. PURCHASE & ADMIN SETTINGS ---
elif menu == "📦 Purchase":
    st.header("📦 Purchase")
    with st.form("pur"):
    ...
    with st.form("bal"):
        b_t = st.selectbox("Update For", ["Cash", "Online"]); b_a = st.number_input("Enter Base Amount")
