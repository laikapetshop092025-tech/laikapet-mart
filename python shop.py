...
# SCRIPT_URL aur SHEET_LINK bilkul wahi hain
...
# --- 4. DASHBOARD (REPORTS & SAFETY LOGIC) ---
if menu == "📊 Dashboard":
    ...
    # SAFETY CHECK: Photo error fix for Line 66 (Purchase)
    ...
    # TODAY REPORT (Photo error fix for Line 115, 121, 125)
    ...
    # MONTHLY REPORT (Photo error fix for Line 133)
...
# --- 5. BAAKI TABS (WORD TO WORD ORIGINAL) ---
elif menu == "🧾 Billing":
    st.header("🧾 Billing")
    ...
    with st.expander("📥 Add Item"):
        ...
        if st.button("➕ Add Item"): st.session_state.pur_cart.append({"Item": n, "Qty": q, "Rate": r}); st.rerun()
    if st.session_state.pur_cart:
        ...
        if st.button("💾 Save All"):
            for item in st.session_state.pur_cart: save_data("Inventory", [item['Item'], item['Qty'], "Pcs", item['Rate'], str(today_dt), p_from])
    ...
    if not i_df.empty:
        ...
        for i, row in stock_sum.iterrows():
            if row.iloc[1] < 2: st.error(f"🚨 ALERT LOW STOCK: {row.iloc[0]} ({row.iloc[1]} Left)")
