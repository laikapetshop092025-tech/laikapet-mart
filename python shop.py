...
# --- 1. SETUP & CONNECTION (FIXED URL) ---
st.set_page_config(page_title="LAIKA PET MART", layout="wide")

# Yeh URL ab ekdam sahi hai, isse data save hone lagega
...
if menu == "📊 Dashboard":
    ...
    galla_cash = bc + sc + kjc - ec - pc - spc
    galla_bank = bo + so + kjo - eo - po - spo
    ...
    <div style="display: flex; gap: 10px; justify-content: space-around;">
        ...
        <div style="background-color: #E3F2FD; padding: 15px; border-radius: 10px; border-left: 8px solid #1976D2; width: 19%;">
            <p style="color: #1976D2; margin: 0;">🏦 Online (Bank)</p> <h3 style="margin: 0;">₹{galla_bank:,.2f}</h3>
        </div>
        <div style="background-color: #F3E5F5; padding: 15px; border-radius: 10px; border-left: 8px solid #7B1FA2; width: 19%;">
            <p style="color: #7B1FA2; margin: 0;">⚡ Cash + Online</p> <h3 style="margin: 0;">₹{galla_cash + galla_bank:,.2f}</h3>
        </div>
    ...
    ts_d = pd.to_numeric(s_t.iloc[:, 3], errors='coerce').sum(); te_d = pd.to_numeric(e_df[e_df['Date'].dt.date == today_dt].iloc[:, 2], errors='coerce').sum() if not e_df.empty else 0
    tprof_d = pd.to_numeric(s_t.iloc[:, 7], errors='coerce').sum() if len(s_t.columns)>7 else 0
    c1, c2, c3 = st.columns(3); c1.metric("Sale", f"₹{ts_d}"); c2.metric("Expense", f"₹{te_d}"); c3.metric("Profit", f"₹{tprof_d}")

# --- 5. BAAKI ALL TABS (NO CHANGES) ---
elif menu == "🧾 Billing":
    st.header("🧾 Billing")
    ...
    if not d_df.empty:
        ...
        for i, row in d_df.iterrows():
            c1, c2 = st.columns([8, 1]); c1.write(f"🏢 {row.iloc[0]} | ₹{row.iloc[1]} ({row.iloc[3]})")
            if c2.button("❌", key=f"d_{i}"): delete_row("Dues", i); st.rerun()
