...
# --- 4. DASHBOARD (FIXED FOR CASH DEDUCTION & REPORTS) ---
if menu == "📊 Dashboard":
    ...
    if not i_df.empty:
        ...
        # Deducting purchase amounts from Cash/Online
    ...
    g_cash = bc + sc + kjc - ec - pc
    g_bank = bo + so + kjo - eo - po
...
# --- BAAKI TABS (BILLING, PURCHASE, STOCK ETC) ---
elif menu == "🧾 Billing":
    st.header("🧾 Billing")
    # ... Billing Logic wahi purana ...
    st.info("Billing Logic Active")
    ...
    # ... Pet Logic wahi purana ...
    st.info("Pet Register Logic Active")

elif menu == "📒 Customer Khata":
    st.header("📒 Customer Khata")
    with st.form("kh"):
        n = st.text_input("Customer Name"); a = st.number_input("Amount"); t = st.selectbox("Type", ["Udhaar (+)", "Jama (-)"]); m = st.selectbox("Mode", ["Cash", "Online", "N/A"])
        if st.form_submit_button("Save"):
            save_data("CustomerKhata", [n, a if "+" in t else -a, str(today_dt), m]); st.rerun()
    k_df = load_data("CustomerKhata")
    if not k_df.empty:
        # 🚨 Zero Balance Logic: Filter out zero entries
        summary = k_df.groupby(k_df.columns[0]).agg({k_df.columns[1]: 'sum'}).reset_index()
        summary = summary[summary.iloc[:, 1] != 0] # Sirf unhe dikhao jinka balance zero nahi hai
        st.subheader("📋 Active Balances")
        st.table(summary)

elif menu == "🏢 Supplier Dues":
    st.header("🏢 Supplier Dues")
    # ... Supplier Logic wahi purana ...
    st.info("Supplier Logic Active")
