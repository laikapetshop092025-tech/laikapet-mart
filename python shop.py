...
# --- 4. COLOURFUL DASHBOARD (WITH FULL STATS) ---
if menu == "📊 Dashboard":
...
# --- 5. BILLING (UNITS & LIST) ---
    ...
    with st.form("bill"):
        ...
        if st.form_submit_button("SAVE BILL"):
            pts = int((q * pr) / 100); save_data("Sales", [str(today_dt), it, f"{q} {unit}", q*pr, mode, ph, pts])
            time.sleep(1); st.rerun()
    s_df = load_data("Sales")
    if not s_df.empty:
        for i, row in s_df.tail(10).iterrows():
            col1, col2, col3 = st.columns([6, 2, 1])
            with col1: st.write(f"*{row.iloc[1]}* | ₹{row.iloc[3]} ({row.iloc[2]})")
            with col2: st.markdown(f"[📲 Bill](https://wa.me/{row.iloc[5]}?text=Bill)")
            if col3.button("❌", key=f"s_{i}"): delete_row("Sales", i); st.rerun()

# --- 6. PET REGISTER (RESTORED LIST) ---
...
# --- 7. CUSTOMER KHATA (RESTORED LIST) ---
    ...
    if not k_df.empty:
        st.subheader("Baki Hisaab (Summary)")
        ...
        st.divider(); st.subheader("Recent History")
...
# --- 8. EXPENSES (RESTORED DROPDOWN & LIST) ---
elif menu == "💰 Expenses":
    st.header("💰 Expenses")
    with st.form("exp"):
...
# --- 9. LIVE STOCK (RESTORED ALERT & DOWNLOAD) ---
...
# --- 10. PURCHASE (WITH UNITS) ---
elif menu == "📦 Purchase":
    st.header("📦 Purchase")
    with st.form("pur"):
        n = st.text_input("Item Name"); c1, c2, c3 = st.columns(3)
        with c1: q = st.number_input("Qty", 1.0)
        with c2: u = st.selectbox("Unit", ["Kg", "Pcs", "Packet"])
...
# --- 11. LOYALTY CLUB & ADMIN SETTINGS ---
    ...
    if not s_df.empty:
        ...
        loyalty.columns = ['Phone', 'Points']; st.dataframe(loyalty, use_container_width=True)
