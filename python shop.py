...
if menu == "📊 Dashboard":
    ...
    <div style="display: flex; gap: 10px; justify-content: space-around;">
        <div style="background-color: #FFEBEE; padding: 15px; border-radius: 10px; border-left: 8px solid #D32F2F; width: 19%;">
            <p style="color: #D32F2F; margin: 0;">💵 Galla</p> <h3 style="margin: 0;">₹{bc + sc - ec:,.2f}</h3>
        </div>
    ...
    ts_d = pd.to_numeric(s_t.iloc[:, 3], errors='coerce').sum(); tprof_d = pd.to_numeric(s_t.iloc[:, 7], errors='coerce').sum() if len(s_t.columns)>7 else 0
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Sale", f"₹{ts_d}"); c2.metric("Profit", f"₹{tprof_d}")

# --- 5. BILLING (ROYALTY CLUB + REDEEM RESTORED) ---
elif menu == "🧾 Billing":
    ...
    st.warning(f"👑 Royalty Club: {c_name} has {pts_bal} Points available.")
...
# --- 6. PURCHASE (WINDOW RESTORED) ---
elif menu == "📦 Purchase":
    st.header("📦 Purchase Entry")
    with st.expander("📥 Add Stock", expanded=True):
        n = st.text_input("Item Name"); q = st.number_input("Qty", 1.0); u = st.selectbox("Unit", ["Kg", "Pcs", "Pkt"]); r = st.number_input("Rate"); m = st.selectbox("Mode", ["Cash", "Online"])
        if st.button("➕ Add Item"):
            save_data("Inventory", [n, f"{q} {u}", "Stock", r, str(today_dt), m]); st.rerun()

# --- 7. LIVE STOCK (RED ALERT + TOTAL AMT) ---
    ...
    if not i_df.empty:
        ...
        for _, row in i_df.iterrows():
            ...
            if qv < 2: st.error(f"🚨 LOW STOCK ALERT: {row.iloc[0]} ({row.iloc[1]} Left)")
            else: st.info(f"✅ {row.iloc[0]}: {row.iloc[1]} Left")

# --- 8. EXPENSES (WINDOW RESTORED) ---
elif menu == "💰 Expenses":
...
# --- 9. PET REGISTER (FULL LOGIC + VAX TRACKER) ---
elif menu == "🐾 Pet Register":
    st.header("🐾 Pet Register & Vaccination Tracker")
    with st.form("pet"):
        c1, c2 = st.columns(2)
        on = c1.text_input("Owner Name"); oph = c2.text_input("Phone Number")
        br = c1.selectbox("Breed", ["Labrador", "GSD", "Pug", "Other"]); age = c2.number_input("Age (Months)", 1)
        wt = c1.number_input("Weight (Kg)", 0.1); vd = c2.date_input("Vaccination Date")
        if st.form_submit_button("Save"):
            nv = vd + timedelta(days=365)
            save_data("PetRecords", [on, oph, br, age, wt, str(vd), str(nv), str(today_dt)]); st.rerun()
...
# --- 10. CUSTOMER KHATA (ZERO BALANCE LOGIC) ---
    ...
    if not k_df.empty:
        ...
        st.subheader("📋 Active Udhaar List")
        st.table(sum_df[sum_df.iloc[:, 1] != 0])

# --- 11. SUPPLIER DUES (WINDOW RESTORED) ---
