...
# --- 4. DASHBOARD (ALL LOGICS RESTORED) ---
if menu == "📊 Dashboard":
    ...
    <div style="display: flex; gap: 10px; justify-content: space-around;">
        <div style="background-color: #FFEBEE; padding: 15px; border-radius: 10px; border-left: 8px solid #D32F2F; width: 19%;">
            <p style="color: #D32F2F; margin: 0;">💵 Cash</p> <h3 style="margin: 0;">₹{bc + sc - ec - pc:,.2f}</h3>
        </div>
        ...
        <div style="background-color: #F3E5F5; padding: 15px; border-radius: 10px; border-left: 8px solid #7B1FA2; width: 19%;">
            <p style="color: #7B1FA2; margin: 0;">⚡ Total</p> <h3 style="margin: 0;">₹{bc+bo+sc+so-ec-eo-pc-po:,.2f}</h3>
        </div>
...
# --- 6. PURCHASE ---
    ...
    st.header("📦 Purchase")
    with st.expander("📥 Add Item", expanded=True):
        n = st.text_input("Item Name"); q = st.number_input("Qty", 1.0); u = st.selectbox("Unit", ["Kg", "Pcs", "Pkt"]); r = st.number_input("Rate"); m = st.selectbox("Paid From", ["Cash", "Online"])
        if st.button("➕ Add Item"):
            save_data("Inventory", [n, f"{q} {u}", "Stock", r, str(today_dt), m]); st.rerun()

# --- 7. LIVE STOCK (TOTAL AMT RESTORED) ---
...
# --- 9. PET REGISTER (FULL DETAILS) ---
    ...
    with st.form("pet"):
        c1, c2 = st.columns(2)
        on = c1.text_input("Owner"); ph = c2.text_input("Phone"); br = c1.selectbox("Breed", ["Labrador", "GSD", "Pug", "Other"])
...
# --- 10. CUSTOMER KHATA (ZERO BALANCE) ---
